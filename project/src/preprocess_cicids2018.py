...
호환 조건 (preprocess_cicids2017.py 와 반드시 일치):
  ...
  - Scaler: CIC2017 학습 시 저장된 scaler 로 transform only (fit 금지)
"""
...
def filter_bot_benign(df: pd.DataFrame) -> pd.DataFrame:
    ...
    bot_mask    = label_lower.str.startswith("botnet")
...
# =========================================================
# 10. CIC-IDS2017 scaler 적용 (transform only — fit 금지)
# =========================================================
def apply_scaler(X: np.ndarray) -> np.ndarray:
    ...
    print(f"[SCALER] 스케일링 완료: {X_scaled.shape}")
    return X_scaled
