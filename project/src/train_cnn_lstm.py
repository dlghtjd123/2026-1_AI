...
_SEQ = {
    "full":   _PROJECT / "data" / "processed" / "cicids" / "seq",
    "common": _PROJECT / "data" / "processed" / "cicids" / "seq_common",
}


def parse_args():
    parser = argparse.ArgumentParser(description="Train CNN-LSTM")
    parser.add_argument(
        "--mode",
        type=str,
        choices=["full", "common"],
...
def main():
    ...
    model = CNNLSTMModel(n_features=n_features, conv_channels=64, lstm_hidden=64, dropout=0.3).to(device)
    ...
    print_metrics(f"CNN-LSTM [{mode}] Validation", best_val_metrics, loss=best_val_loss)
    ...
    torch.save(
        ...
        MODEL_DIR / f"cnn_lstm_{mode}.pt",
    )

    with open(MODEL_DIR / f"cnn_lstm_{mode}_threshold.json", "w", encoding="utf-8") as f:
        json.dump({"threshold": best_threshold, "mode": mode}, f, indent=4, ensure_ascii=False)

    with open(RESULT_DIR / f"cnn_lstm_{mode}_val_metrics.json", "w", encoding="utf-8") as f:
        json.dump(best_val_metrics, f, indent=4, ensure_ascii=False)

    print(f"\n[SAVED] artifacts/models/cnn_lstm_{mode}.pt")
    print(f"[SAVED] artifacts/models/cnn_lstm_{mode}_threshold.json")
    print(f"[SAVED] artifacts/results/cnn_lstm_{mode}_val_metrics.json")
