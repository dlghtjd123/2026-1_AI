...
DATA_DIR   = _PROJECT / "data" / "processed" / _DATASET_KEY / "seq"
...
MODEL_DIR  = _ROOT / "artifacts" / f"models_{_DATASET_KEY}{_MODEL_SUFFIX}" / "cnn_lstm"
...
def prepare_train_data(X, y, fold_id=None):
    ...
    X, y = augment_train_fold(
        ...
        feature_names=json.loads(FEATURE_PATH.read_text(encoding="utf-8")).get("features") if FEATURE_PATH.exists() else None,
    )
...
def print_fold_summary(fold_results: list[dict]) -> dict:
    ...
    summary = {}
    print(f"\n{'='*60}")
    print(f"  CNN-LSTM  {N_FOLDS}-Fold CV 결과 요약  [{DATASET} / {AUGMENT}]")
...
def main():
    ...
    print(f"\n[SAVED] {MODEL_DIR}/cnn_lstm_flow.pt")
    print(f"[SAVED] {MODEL_DIR}/cnn_lstm_flow_threshold.json")
    print(f"[SAVED] {RESULT_DIR}/cnn_lstm_flow_kfold_results.json")
