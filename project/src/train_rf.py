# =========================================================
# Random Forest 이진 분류 모델 학습 스크립트
# ---------------------------------------------------------
# 윈도우 플래튼(window-flattened) 형태로 전처리된 네트워크
# 트래픽 데이터를 입력받아 Random Forest 분류기를 학습한다.
#
# 주요 기능:
#   - 클래스 불균형 대응: class_weight="balanced_subsample" 적용
#   - 최적 임계값 탐색: F1 > Recall > Precision 우선순위로 선정
#   - 학습된 모델/임계값/평가 지표를 파일로 저장
#
# 입력: data/processed/winflat/{X_train, y_train, X_val, y_val}.npy
# 출력: artifacts/models/rf_model.pkl
#       artifacts/models/rf_threshold.json
#       artifacts/results/rf_val_metrics.json
# =========================================================
...
# 경로 설정
# ---------------------------------------------------------
# 전처리된 윈도우 플래튼 데이터(.npy), 학습된 모델 가중치(.pkl),
...
#    - n_estimators=500 : 트리 500개로 앙상블 구성
#    - max_depth=20     : 과적합 방지를 위한 트리 깊이 제한
#    - max_features="sqrt" : 분기 시 sqrt(n_features) 개만 고려
#    - class_weight="balanced_subsample" : 부트스트랩 샘플 단위로
#      클래스 불균형 자동 보정
# 3. 검증 데이터로 예측 확률 산출 후 최적 임계값 탐색
# 4. 성능 지표 출력
# 5. 모델/임계값/지표 파일 저장
# =========================================================
def main():
